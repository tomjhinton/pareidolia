    ...
    inputs.forEach((midiInput) => {
      ...
      inputText.push(`FOUND: ${midiInput.name}`);
        midiInput.onmidimessage = function(message) {
          if(material.uniforms[`u_${message.data[1]}`]){
          material.uniforms[`u_${message.data[1]}`].value = parseFloat(message.data[2])
        } else{
            material.uniforms[`u_${message.data[1]}`] = {
              value: parseFloat(message.data[2])
            }
        }
